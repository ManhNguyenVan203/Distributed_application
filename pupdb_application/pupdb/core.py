...
class PupDB(object):
    ...
    def replicate_to_slaves(self, key, val):
        """ Nhân bản dữ liệu tới các nút phụ. """
        for slave in self.slave_nodes:
            try:
                slave.set(key, val)
            except Exception as e:
                logging.error(f'Error replicating to slave {slave}: {e}')

    def set(self, key, val):
        """ Ghi dữ liệu và nhân bản tới các nút phụ. """
        super().set(key, val)  # Ghi trên nút chính
        self.replicate_to_slaves(key, val)  # Nhân bản tới nút phụ